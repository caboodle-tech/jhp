...
`index.jhp` includes partials with paths that start with `/` (for example `/partials/header.html`). The example `build.js` sets `rootDir` to this `src` directory so that those paths are rooted here, not at the real filesystem root.