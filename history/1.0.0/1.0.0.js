...
class JSHypertextPreprocessor {
    ...
    #dollar = {
        ...
        include: (file) => this.#include(file),
        obClose: () => {
        ...
        obStatus: () => this.#htmlOutputBuffer.open,
        version: () => `JHP Version ${VERSION}`
    };
    ...
    #tokenReplacement(code) {
        return code.replace(this.#regex.preprocess, (match, key) => (key in this.#dollar ? `$.${key}` : match));
    }
